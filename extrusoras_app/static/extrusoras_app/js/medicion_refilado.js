$(document).ready(function() {
	...
	$("[name=mef_metro],[name=mef_ancho_plano]").on("keypress",floatNumber);

	//Habilitando campos
	$("[name=mef_metro],[name=mef_ancho_plano]").each(function(){
		if(($(this).attr("name")=="mef_metro" && $(this).attr("id").split("_")[3]!=1) || ($(this).attr("name")=="mef_ancho_plano" && $(this).attr("id").split("_")[4]!=1))
		{
			$(this).prop("disabled",true);
		}
	});
	$("[name=mef_ancho_plano]").on("blur",activarNuevoRegistro);	
});
function activarNuevoRegistro(){
	...
	if(nro<qtyMediciones){
		if($.trim($("#mef_metro_"+maq+"_"+nro).val())!="" && $.trim($("#mef_ancho_plano_"+maq+"_"+nro).val())!=""){
			nro = nro + 1;
			$("#mef_metro_"+maq+"_"+nro).prop("disabled",false);
			$("#mef_ancho_plano_"+maq+"_"+nro).prop("disabled",false);
			if(nro>2){
				$("#mef_metro_"+maq+"_"+nro).focus();
			}
		}
	}
}
...
function guardarMedicion(obj){
	...
	if($.trim($('#bob_id_'+maq).val())!="" && $.trim($('#bob_numero_'+maq).val())!=""){
		...
		if($.trim($('#mef_metro_'+maq+'_1').val())=="" || $.trim($('#mef_ancho_plano_'+maq+'_1').val())==""){
		...
		let intFields = ['#mef_metro_'+maq+'_1', '#mef_ancho_plano_'+maq+'_1',];
		...
		for(qM=2;qM<=qtyMediciones;qM++){
			if((valido=='si') && ($.trim($('#mef_metro_'+maq+'_'+qM).val())=="" || $.trim($('#mef_ancho_plano_'+maq+'_'+qM).val())=="")){
			...
			intFields.push('#mef_metro_'+maq+'_'+qM,'#mef_ancho_plano_'+maq+'_'+qM);
		...
		datos.mef_metro = $('#mef_metro_'+maq+'_1').val()+'|';
		datos.mef_ancho_plano = $('#mef_ancho_plano_'+maq+'_1').val()+'|';
		for(qM=2;qM<=qtyMediciones;qM++){
			datos.mef_metro+=$('#mef_metro_'+maq+'_'+qM).val()+'|';
			datos.mef_ancho_plano+=$('#mef_ancho_plano_'+maq+'_'+qM).val()+'|';
		}
		$.ajax({
	        url: '/extrusoras_app/ajax/guardarMedicionRefilado/',
	        dataType: 'json',
	        type: 'POST',
	        data: datos,
	        success: function (data) {
						if(data["medicion"].length>0){
							...
							for(j=0;j<data["medicion"].length;j++){
								let $row = rowMedicionBobina($('#bob_id_'+maq).val(),
									...
									data["medicion"][j].ancho);
								$('#historial_medicion_'+maq+' #tabla_mediciones').append($row);
							}
							$('[id^=mef_metro_'+maq+'], [id^=mef_ancho_plano_'+maq+']').val("");
							$('[id^=mef_metro_'+maq+'], [id^=mef_ancho_plano_'+maq+']').each(function(){
								if(($(this).attr("name")=="mef_metro" && $(this).attr("id").split("_")[3]!=1) || ($(this).attr("name")=="mef_ancho_plano" && $(this).attr("id").split("_")[4]!=1))
								{
									$(this).prop("disabled",true);
								}
...
function rowMedicionBobina(id_bobina, nro, fecha, hora, medida, ancho){
	let $row = '<tr>';
	$row += '<td>'+id_bobina+'</td>';
	$row += '<td>'+nro+'</td>';
	$row += '<td>'+fecha+'</td>';
...
function cerrarProceso(){
	...
	if($.trim($('#rem_id_'+maq).val())!="" && $.trim($('#rem_id_'+maq).val())!="0"){
		$.ajax({
			url: '/extrusoras_app/ajax/finalizarOrdenRefilado/',
			dataType: 'json',
			type: 'POST',
			data:{
				'rem_id': $('#rem_id_'+maq).val(),
...
function imprimeEtiqueta(){
	...
	window.open('/extrusoras_app/etiqueta_refilado/'+$.trim($('#bob_id_'+maq).val())+'/'+$.trim($('#rem_id_'+maq).val())+'/','etiquetaRefilado','toolbar=yes,location=yes,status=yes');
